...
Linux.FrameTransparency = 0.75
Linux.FrameColor = Color3.fromRGB(55, 55, 59)
Linux.MainFrameColor = Color3.fromRGB(13, 14, 15)
...
function Linux.RepositionNotifications()
    ...
    for i, notificationData in ipairs(Linux.ActiveNotifications) do
        if notificationData.Frame and notificationData.Frame.Parent then
            ...
            local targetPosition = UDim2.new(1, -(isMobile and 210 or 320), 1, -yOffset - notificationData.Frame.Size.Y.Offset)
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
...
function Linux:Notify(config)
    ...
    local titleSize = TextService:GetTextSize(titleText, 16, Enum.Font.GothamSemibold, Vector2.new(9999, 9999))
    local contentSize = TextService:GetTextSize(contentText, 14, Enum.Font.GothamSemibold, Vector2.new(9999, 9999))
    local subContentSize = subContentText ~= "" and TextService:GetTextSize(subContentText, 12, Enum.Font.GothamSemibold, Vector2.new(9999, 9999)) or Vector2.new(0, 0)
    
    local baseWidth = isMobile and 200 or 300
    local maxTextWidth = math.max(titleSize.X, contentSize.X, subContentSize.X)
    local notificationWidth = math.max(baseWidth, math.min(maxTextWidth + logoOffset + 60, 400))
    ...
    local success, result = pcall(function()
        if listfiles and isfolder(Linux.ConfigFolder) then
            ...
            for _, file in pairs(files) do
                ...
                if configName and configName ~= "autoload" and configName ~= "privacy" and configName ~= "accent" and configName ~= "resize" then
...
function Linux.Create(config)
    ...
    local connection7 = globalConnectionManager:addConnection(ResizeHandle.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            isResizing = false
            Linux.SaveResizeSettings()
        end
    ...
    local connection8 = globalConnectionManager:addConnection(InputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if isResizing then
                isResizing = false
                Linux.SaveResizeSettings()
                ...
                for _, elementData in pairs(Tabs[tabIndex].Elements) do
                    if elementData.Type == "Slider" then
                        ...
                        if fillBar and knob then
                            ...
                            local tween2 = TweenService:Create(knob, tween, {Position = UDim2.new(targetPos, -6, 0.5, -6)})
                            globalConnectionManager:addTween(tween1)
                            globalConnectionManager:addTween(tween2)
                            tween1:Play()
                            tween2:Play()
    ...
    function LinuxLib.Tab(config)
        ...
        local CollapseCountTab = config.CollapseCountTab or 1
        ...
        if SectionText and SectionText ~= "" then
            ...
            local SectionButton = Linux.Instance("TextButton", {
                Parent = SectionFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 1, 0),
                Text = "",
            ...
            Linux.TabSections[tabIndex] = {
                SectionFrame = SectionFrame,
                SectionIcon = SectionIcon,
                SectionLabel = SectionLabel,
                CollapseEnable = CollapseEnable,
                CollapseCountTab = CollapseCountTab,
                IsCollapsed = true,
                TargetTabs = {}
            }
            
            local function toggleSectionCollapse()
                if not CollapseEnable then return end
                
                local sectionData = Linux.TabSections[tabIndex]
                sectionData.IsCollapsed = not sectionData.IsCollapsed
                
                if SectionIcon then
                    local tween = TweenService:Create(SectionIcon, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Rotation = sectionData.IsCollapsed and 0 or 90
                    })
                    globalConnectionManager:addTween(tween)
                    tween:Play()
                end
                ...
                local tabsToHide = {}
                local tabsFound = 0
                local foundCurrentTab = false
                
                for i = tabOrder, #Linux.TabSections + tabOrder do
                    local tab = Tabs[i]
                    if tab then
                        if tab.SectionData and tab.SectionData.tabIndex == tabIndex then
                            foundCurrentTab = true
                            continue
                        end
                        
                        if foundCurrentTab and tabsFound < CollapseCountTab then
                            table.insert(tabsToHide, tab)
                            tabsFound = tabsFound + 1
                        elseif foundCurrentTab and tabsFound >= CollapseCountTab then
                            break
                        end
                    end
                end
                
                sectionData.TargetTabs = tabsToHide
                
                for _, tab in pairs(tabsToHide) do
                    if tab.Button then
            ...
            globalConnectionManager:addConnection(SectionButton.MouseButton1Click:Connect(toggleSectionCollapse))
        ...
        local function SelectTab()
            ...
            if CurrentTab and Tabs[CurrentTab] then
                ...
                oldTab.Text.TextColor3 = Color3.fromRGB(210, 210, 210)
            ...
            TabText.TextColor3 = Color3.fromRGB(210, 210, 210)
            TabBtn.BackgroundColor3 = Color3.fromRGB(55, 55, 57)
        ...
        function TabElements.Button(config)
            ...
            if CollapseEnable then
                ...
                local CollapseGradient = Linux.Instance("UIGradient", {
                    ...
                    Color = ColorSequence.new({
                        ColorSequenceKeypoint.new(0, Linux.Accents[Linux.CurrentAccent]),
                        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 255, 255)),
                        ColorSequenceKeypoint.new(1, Linux.Accents[Linux.CurrentAccent])
                    }),
            ...
            globalConnectionManager:addConnection(Btn.MouseEnter:Connect(function()
                local tween1 = TweenService:Create(BtnFrame, TweenInfo.new(0.2), {BackgroundTransparency = 0.3})
                local tween2 = TweenService:Create(HoverEffect, TweenInfo.new(hoverDuration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Size = UDim2.new(1, 0, 1, 0),
            ...
            globalConnectionManager:addConnection(Btn.MouseLeave:Connect(function()
                local tween1 = TweenService:Create(BtnFrame, TweenInfo.new(0.2), {BackgroundTransparency = Linux.FrameTransparency})
                local tween2 = TweenService:Create(HoverEffect, TweenInfo.new(hoverDuration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Size = UDim2.new(0, 0, 1, 0),
                    BackgroundTransparency = 1
                })
                ...
                for i, element in pairs(targetElements) do
                    if element and element.Parent then
                        if IsCollapsed then
                            ...
                            local showTween = TweenService:Create(element, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                                Position = UDim2.new(0, 0, 0, 0)
                            })
                            globalConnectionManager:addTween(showTween)
                            showTween:Play()
                        end
                    end
                end
            ...
            local function createConfirmationDialog(confirmationConfig)
                ...
                globalConnectionManager:addTween(dialogTween)
                dialogTween:Play()
                ...
                local function closeDialog()
                    ...
                    globalConnectionManager:addTween(closeTweenDialog)
                            ...
                            if i == 1 then
                                if Linux.CurrentAccent == "Black & White" then
                                    ...
                                    hoverColor = Color3.fromRGB(
                                        math.min(255, Linux.Accents[Linux.CurrentAccent].R * 255 + 20),
                                        math.min(255, Linux.Accents[Linux.CurrentAccent].G * 255 + 20),
                                        math.min(255, Linux.Accents[Linux.CurrentAccent].B * 255 + 20)
                                    )
                                end
                            else
                                hoverColor = Color3.fromRGB(80, 80, 90)
                            end
                            
                            local tween = TweenService:Create(DialogButton, TweenInfo.new(0.2), {BackgroundColor3 = hoverColor})
                            globalConnectionManager:addTween(tween)
                            tween:Play()
                        end))
                        
                        globalConnectionManager:addConnection(DialogButton.MouseLeave:Connect(function()
                            local tween = TweenService:Create(DialogButton, TweenInfo.new(0.2), {
                                BackgroundColor3 = dialogButtonColor
                            })
                            globalConnectionManager:addTween(tween)
                            tween:Play()
        ...
        function TabElements.Dropdown(config)
            ...
            local function UpdateDropSize()
                ...
                if IsOpen then
                    ...
                    tween1:Play()
                    if #Options > 0 then
                        local tween2 = TweenService:Create(Arrow, tween, {Rotation = 85})
                        globalConnectionManager:addTween(tween2)
                        tween2:Play()
                    end
                else
        ...
        function TabElements.Slider(config)
            ...
            local function UpdateSlider(pos)
                ...
                local tween2 = TweenService:Create(SliderKnob, tween, {Position = UDim2.new(relativePos, -6, 0.5, -6)})
                globalConnectionManager:addTween(tween1)
                globalConnectionManager:addTween(tween2)
                tween1:Play()
                tween2:Play()
        ...
        function TabElements.Input(config)
            ...
            local TextBox = Linux.Instance("TextBox", {
                Parent = TextBoxFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, -12 - iconOffset, 1, 0),
                Position = UDim2.new(0, 6 + iconOffset, 0, 0),
                Font = Enum.Font.GothamSemibold,
        ...
        function TabElements.Keybind(config)
            ...
            local KeybindText = Linux.Instance("TextLabel", {
                Parent = Keybind,
                BackgroundTransparency = 1,
            ...
            local Mode = config.Mode or "Always"
            local CurrentKey = config.Default or Enum.KeyCode.E
            local IsListening = false
            local IsActive = false
            local HoldConnection = nil
            
            local ModeFrame = Linux.Instance("Frame", {
                Parent = Keybind,
                BackgroundColor3 = Color3.fromRGB(14, 14, 14),
                BackgroundTransparency = 0.6,
                BorderSizePixel = 0,
            ...
            local ModeLabel = Linux.Instance("TextLabel", {
                Parent = ModeFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 1, 0),
                Font = Enum.Font.GothamSemibold,
            ...
            local KeyFrame = Linux.Instance("Frame", {
                Parent = Keybind,
                BackgroundColor3 = Color3.fromRGB(14, 14, 14),
                BackgroundTransparency = 0.6,
                BorderSizePixel = 0,
            ...
            local KeyLabel = Linux.Instance("TextLabel", {
                Parent = KeyFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 1, 0),
                Font = Enum.Font.GothamSemibold,
            ...
            local KeyButton = Linux.Instance("TextButton", {
                Parent = KeyFrame,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 1, 0),
                Text = "",
            ...
            local StatusIndicator = Linux.Instance("Frame", {
                Parent = Keybind,
            ...
            local function UpdateStatusIndicator()
                local targetColor = IsActive and Linux.Accents[Linux.CurrentAccent] or Color3.fromRGB(60, 60, 70)
                local tween = TweenService:Create(StatusIndicator, TweenInfo.new(0.2), {BackgroundColor3 = targetColor})
                globalConnectionManager:addTween(tween)
                tween:Play()
            end
            ...
            local function GetKeyName(keyCode)
                ...
                if keyName == "LeftShift" then return "LShift"
                elseif keyName == "RightShift" then return "RShift"
                elseif keyName == "LeftControl" then return "LCtrl"
                ...
                elseif keyName == "Space" then return "Space"
                elseif keyName == "Return" then return "Enter"
                elseif keyName == "Backspace" then return "Back"
                elseif keyName == "Delete" then return "Del"
                elseif keyName == "Tab" then return "Tab"
                elseif keyName == "CapsLock" then return "Caps"
                elseif keyName == "Escape" then return "Esc"
                elseif keyName == "Insert" then return "Ins"
                elseif keyName == "Home" then return "Home"
                elseif keyName == "End" then return "End"
                elseif keyName == "PageUp" then return "PgUp"
                elseif keyName == "PageDown" then return "PgDn"
                elseif keyName == "Up" then return "↑"
                elseif keyName == "Down" then return "↓"
                elseif keyName == "Left" then return "←"
                elseif keyName == "Right" then return "→"
                elseif string.match(keyName, "^F%d+$") then return keyName
                elseif string.match(keyName, "^KeypadZero$") then return "Num0"
                elseif string.match(keyName, "^KeypadOne$") then return "Num1"
                elseif string.match(keyName, "^KeypadTwo$") then return "Num2"
                elseif string.match(keyName, "^KeypadThree$") then return "Num3"
                elseif string.match(keyName, "^KeypadFour$") then return "Num4"
                elseif string.match(keyName, "^KeypadFive$") then return "Num5"
                elseif string.match(keyName, "^KeypadSix$") then return "Num6"
                elseif string.match(keyName, "^KeypadSeven$") then return "Num7"
                elseif string.match(keyName, "^KeypadEight$") then return "Num8"
                elseif string.match(keyName, "^KeypadNine$") then return "Num9"
                elseif string.match(keyName, "^KeypadPeriod$") then return "Num."
                elseif string.match(keyName, "^KeypadDivide$") then return "Num/"
                elseif string.match(keyName, "^KeypadMultiply$") then return "Num*"
                elseif string.match(keyName, "^KeypadMinus$") then return "Num-"
                elseif string.match(keyName, "^KeypadPlus$") then return "Num+"
                elseif string.match(keyName, "^KeypadEnter$") then return "NumEnter"
                else
                    return keyName
                end
            end
            ...
            local function ActivateKeybind()
                if Mode == "Always" then
                    IsActive = true
                    spawn(function() Linux:SafeCallback(config.Callback, true) end)
                elseif Mode == "Toggle" then
                    IsActive = not IsActive
                    spawn(function() Linux:SafeCallback(config.Callback, IsActive) end)
            ...
            local function DeactivateKeybind()
                if Mode == "Hold" then
                    IsActive = false
                    spawn(function() Linux:SafeCallback(config.Callback, false) end)
                    UpdateStatusIndicator()
                end
            end
            
            globalConnectionManager:addConnection(UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            globalConnectionManager:addConnection(UserInputService.InputEnded:Connect(function(input, gameProcessed)
                if gameProcessed or IsListening then return end
            ...
            globalConnectionManager:addConnection(KeyButton.MouseButton1Click:Connect(function()
                ...
                local keyStroke = KeyFrame:FindFirstChild("UIStroke")
                if keyStroke then
                    local tween = TweenService:Create(keyStroke, TweenInfo.new(0.2), {Color = Linux.Accents[Linux.CurrentAccent]})
                    globalConnectionManager:addTween(tween)
                    tween:Play()
                end
                
                local connection
                connection = globalConnectionManager:addConnection(UserInputService.InputBegan:Connect(function(input, gameProcessed)
                    ...
                    if input.UserInputType == Enum.UserInputType.Keyboard then
                        if input.KeyCode ~= Enum.KeyCode.Unknown then
                            ...
                            if keyStroke then
                                local tween = TweenService:Create(keyStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(30, 30, 30)})
                                globalConnectionManager:addTween(tween)
                                tween:Play()
                            end
    ...
    if config.ConfigSave ~= false then
        ...
        SettingsTab.Button({
            ...
            Callback = function()
                if Linux.CurrentConfig and Linux.CurrentConfig ~= "" then
                    local success, result = Linux.SaveConfig(Linux.CurrentConfig)
                    if success then
                        Linux.LoadConfigList()
                        configListDropdown.SetOptions(Linux.SavedConfigs)
                    else
                        print("Failed to overwrite config:", result)
                    end
                else
                    print("No config selected to overwrite")
                end
            end
        })
